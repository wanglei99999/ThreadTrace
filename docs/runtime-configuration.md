...
In PostgreSQL mode, diagnostics create or use the configured PostgreSQL client, run a lightweight `select 1 as ok` ping, and verify the required ThreadTrace extensions, tables, critical columns, and indexes exist. Missing client configuration, missing `pg`, connection errors, missing `pg_trgm`, missing schema tables, missing notification outbox archive columns, or missing baseline indexes are reported as `fail` resource checks so readiness probes can block traffic before workers start writing production data.
