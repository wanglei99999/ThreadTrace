...
2. New events start with:
   - `deliveryStatus: "pending"`
   - `deliveryAttempts: 0`
   - `nextDeliveryAt: createdAt`
3. `dispatchPendingNotificationEvents` loads due `pending` and `failed` events.
4. Successful delivery changes the event to:
   - `deliveryStatus: "delivered"`
   - `lastDeliveredAt`
   - `nextDeliveryAt: undefined`
5. Failed delivery changes the event to:
