...
- `summary`: source counts, running counts, stale running counts, and disable-blocked counts.
- `blockedDisables`: sources that currently need waiting or an explicit force disable.
- `sources`: per-source lifecycle state, disable guard, latest lifecycle task, and next action.
