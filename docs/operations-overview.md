...
The OpenAPI document exposes these fields through `OperationalOverview`, `WorkerRun`, `WorkerLeaseSummary`, `WorkerLease`, `SourceScope`, `AuthorReviewQueueSummary`, `AuthorReviewQueueSourceHotspot`, and `AuthorReviewQueueItem` schemas for generated clients and external monitoring integrations.
