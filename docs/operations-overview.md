...
The OpenAPI document exposes these fields through `OperationalOverview`, `WorkerRun`, `WorkerLeaseSummary`, `WorkerLease`, `SourceScope`, `AuthorReviewQueueSummary`, `AuthorReviewQueueSourceHotspot`, `AuthorReviewQueueItem`, `OperationsRunbook`, `RunbookAction`, `NotificationEvent`, and `RunbookNotificationEventSynthesisResult` schemas for generated clients and external monitoring integrations.
