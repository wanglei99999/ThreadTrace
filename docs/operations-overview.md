...
`source-diagnostics` emits source-scoped `nextActions` with `evidenceSummary`, including missing handler, adapter, disabled-source, and handler-required location field evidence. The runbook expands those into `sourceDiagnostics.*.<sourceId>` actions with the same source id, command list, structured evidence, and compact evidence summary, while keeping the generic checklist failure for the overall readiness signal. Deployment checklist summaries preserve the first action details so higher-level gates can show the broken stored source instead of only reporting a generic source readiness failure.
...
Runbook notification synthesis promotes critical and warning runbook actions into the notification outbox as `runbook-action` events. It defaults to dry-run through CLI and HTTP, uses stable IDs based on action keys to avoid duplicate alerts, preserves pending/failed delivery state when an action is refreshed, scopes stale resolution to the requested `sourceId` or `sourceKey` when provided, marks stale actions as `resolved`, reopens system-resolved actions if they return, and skips operator-acknowledged or delivered actions so operator decisions remain durable.
