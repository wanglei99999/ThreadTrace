...
`source-diagnostics` emits source-scoped `nextActions` with `evidenceSummary`, including missing handler, adapter, disabled-source, and handler-required location field evidence. Deployment checklist summaries preserve the first action details so higher-level gates can show the broken stored source instead of only reporting a generic source readiness failure.
