...
The OpenAPI document exposes these fields through `OperationalOverview`, `WorkerRun`, `WorkerLeaseSummary`, `WorkerLease`, `SourceScope`, `AuthorReviewQueueSummary`, `AuthorReviewQueueSourceHotspot`, `AuthorReviewQueueItem`, `OperationsRunbook`, `RunbookAction`, `NotificationEvent`, `NotificationEventListResult`, `NotificationEventOverview`, `NotificationEventSourceHotspot`, `NotificationEventAttention`, and `RunbookNotificationEventSynthesisResult` schemas for generated clients and external monitoring integrations.
