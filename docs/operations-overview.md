...
node src/presentation/cli/threadtrace.js source-cockpit-action-plan --rank 1
node src/presentation/cli/threadtrace.js source-collection-health --source-key nga
node src/presentation/cli/threadtrace.js automation-readiness --source-task-mode insight-pipeline
...
GET /api/operations/source-collection-health?sourceId=tracked-source-nga-001
GET /api/operations/automation-readiness?sourceTaskMode=insight-pipeline
...
The system view includes a source operations panel that combines `/api/sources/schedule`, `/api/sources/lifecycle`, and `/api/operations/runbook`. It highlights due sources, skipped/backoff reasons, disable guards, lifecycle attention items, source-scoped runbook actions, collection status groups, collection plans, cursor watermarks, replay evidence, and review action audit totals. The `Automation readiness` button calls `/api/operations/automation-readiness` and shows the v0.2 unattended gates, representative source health, worker commands, LLM provider status, and demo-closure readiness in one place. The `Operator queue` section is backed by `/api/operations/source-cockpit`; it merges schedule, lifecycle, runbook, source attention, and source type operations into ranked work items with severity, priority score, source/source-type scope, drill-down controls, runnable source actions, and suggested commands. Each queue row can open `/api/operations/source-cockpit/action-plan`, which turns the selected item into a side-effect-free plan with API/CLI steps, dry-run previews, execute actions, destructive flags, and source/source-type scoped controls. The `Source attention` section merges schedule, lifecycle, and runbook signals by source so operators can handle the highest-priority source first without mentally joining separate tables. Each source row includes `Ops` and `Health` controls: `Ops` opens `/api/operations/source-drilldown` for the full source timeline, while `Health` opens `/api/operations/source-collection-health` for scheduled-collection readiness across schedule, retry/backoff, cursor, incremental diff, replay evidence, task/event pressure, worker runs, and leases. Operators can run a source, run its insight pipeline, run due collection batches, run due insight batches, dry-run/execute enablement changes, reset failed sources, inspect review action audits, synthesize runbook notification events, synthesize source attention notification events, filter notification events by acknowledgement, delivery status, source key, or event type, inspect notification synthesis policy defaults, bulk-acknowledge the open events in the current filter window, and dry-run/execute handled-event archive retention from the same panel while preserving the dry-run and confirmation boundaries used by the CLI and HTTP APIs. Due collection batch results show the durable batch task, batch trace, per-source task/trace links, pipeline ingest child task links, skipped-source reasons, and source health/drill-down buttons so the operator can move from trigger to evidence without leaving the cockpit. Rollout readiness cards can launch an apply dry-run directly from the reviewed manifest. When an operator selects `execute=true` in the Web apply form, the UI runs `/api/deployment/gate` first, blocks failing gates before submitting apply, requires confirmation for warning gates, and then relies on `/api/operations/rollout-manifest/apply` to create the durable rollout apply task audit and rollback guidance.

CLI operators can use `source-attention-report` for the same source-prioritized view in scripts or terminals. It calls the same application report as `/api/operations/source-attention`, prints source-level signals, priority score, rank, and suggested commands, supports machine-readable output with `--json true`, exits `1` for warning attention, and exits `2` when critical source attention is present. Source attention items include `priorityScore`, `attentionRank`, `recommendedNextAction`, and `recommendedCommand` so Web panels, workers, and external automation can agree on which source should be handled first. Use `source-cockpit-action-plan --rank 1` when an operator or script needs the same API/CLI dry-run and execute plan that the Web Operator queue shows for the top cockpit item.

Runtime:

```js
runtime.getOperationalOverview({ limit: 100 })
runtime.getSourceOperationsCockpit({ limit: 25 })
runtime.getSourceOperationsCockpitActionPlan({ rank: 1 })
runtime.getSourceOperationsDrilldown({ sourceId: 'tracked-source-nga-001' })
runtime.getSourceCollectionHealthProfile({ sourceId: 'tracked-source-nga-001' })
runtime.getAutomationReadinessPlan({ sourceTaskMode: 'insight-pipeline' })
...
runtime.synthesizeSourceAttentionNotificationEvents({ execute: false })
runtime.synthesizeSourceTypeOperationsNotificationEvents({ execute: false })
...
The operations worker runs due-source work, optional review action dry-runs, optional runbook event synthesis, optional source attention event synthesis, optional source type operations event synthesis, optional context review result event synthesis, optional author review queue event synthesis, notification event dispatch, optional handled-event archive retention, overview logging, and source attention summarization in one non-overlapping loop. By default the source step is ingest-only and review/runbook/source-attention/source-type-operations/review-result/author-queue/archive synthesis is off. Each completed worker run stores a compact `sourceAttention` output with status, counts, signal distribution, and the top source-scoped attention items, so run history can be inspected without joining schedule, lifecycle, and runbook reports by hand. Set `--source-task-mode insight-pipeline` or `THREADTRACE_SOURCE_TASK_MODE=insight-pipeline` to run the full source insight pipeline for due sources. Set `--review-action true` to create an audited `context-review-action-apply` dry-run task during a worker run. Real review-action execution requires a runtime `contextReviewActionExecutor` port with `closeTasks` and `mergeContext`; see `docs/review-action-executor.md`. Set `--runbook-events true` to dry-run synthesis during a worker run, or `--runbook-events-execute true` to persist runbook events before dispatch. Set `--source-attention-events true` to dry-run source attention alert synthesis during a worker run, or `--source-attention-events-execute true` to persist `source-attention` events before dispatch. Use `--source-attention-resolve-stale false` to leave existing source attention events open during partial maintenance windows, and `--priority-score-threshold` to tune when lower-severity source attention becomes alert-worthy. Set `--source-type-operations-events true` to dry-run connector-family alert synthesis during a worker run, or `--source-type-operations-events-execute true` to persist `source-type-operations` events before dispatch. Use `--source-type` to target one connector family, `--source-type-operations-resolve-stale false` during partial maintenance windows, and `--source-type-operations-include-readiness-warnings true` when catalog-only readiness warnings should page operators. Set `--context-review-result-events true` to dry-run context review result alert synthesis during a worker run, or `--context-review-result-events-execute true` to persist `context-review-result` events before dispatch. `--source-key` / `--forum` and `--source-id` scope source attention synthesis, context review result synthesis, review-action planning, overview logging, and source attention reporting to one source when supplied. Set `--author-review-queue-events true` to dry-run author queue alert synthesis during a worker run, or `--author-review-queue-events-execute true` to persist `author-review-queue` events before dispatch. Set `--archive-events true` to dry-run handled-event retention after dispatch, or `--archive-events-execute true` to archive acknowledged `delivered` / `resolved` events before the overview snapshot. Optional author queue filters include `--source-key`, `--source-thread-id`, `--author-review-queue-status`, `--author-review-queue-type`, `--author-review-queue-priority`, and `--author-review-queue-resolve-stale false`. Optional archive filters include `--source-key`, `--source-id`, `--archive-event-type`, `--archive-delivery-statuses`, `--archive-older-than-days`, `--archive-scan-limit`, and `--archive-limit`. Set `THREADTRACE_SOURCE_RUN_STALE_AFTER_MS` or `--source-run-stale-after-ms` to control stuck source-run recovery. Failed sources use exponential retry backoff by default: first retry after 60 seconds, capped at one hour. Tune this with `THREADTRACE_SOURCE_FAILURE_RETRY_BACKOFF_MS`, `THREADTRACE_SOURCE_FAILURE_MAX_RETRY_BACKOFF_MS`, `--source-failure-retry-backoff-ms`, or set the first value to `0` to disable failure backoff. This is useful for local deployments or a single background service process. Larger deployments can still run the due-source and event workers separately.
...
Source attention notification synthesis promotes high-priority source attention rows into the notification outbox as `source-attention` events. It defaults to dry-run through CLI, HTTP, Web UI, and operations-worker flags, uses stable IDs based on the attention key plus `sourceId` / `sourceKey` scope, alerts on critical/warning attention and on rows above the configured `priorityScoreThreshold`, preserves pending/failed delivery state when a source row is refreshed, scopes stale resolution to the requested source filters, reopens system-resolved attention if the same source signal returns, and skips operator-acknowledged or delivered events for audit safety.

Source type operations notification synthesis promotes connector-family pressure into the notification outbox as `source-type-operations` events. It defaults to dry-run through CLI, HTTP, Web UI, and operations-worker flags, uses stable IDs based on `sourceType`, alerts on failing source type status, lifecycle pressure, warning/critical source attention, or rows above the configured `priorityScoreThreshold`, ignores pure readiness warnings by default, reopens system-resolved source type events if the same pressure returns, and skips operator-acknowledged or delivered events for audit safety.
