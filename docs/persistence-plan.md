...
| `NotificationEventActionExecutionRepository` | `createFileNotificationEventActionExecutionRepository` | `notification_event_action_executions` |
| `AuthorReviewQueueRepository` | `createFileAuthorReviewQueueRepository` | `author_review_queue_items` |
...
- `notification_event_action_executions` is the idempotency ledger for operator-triggered notification event actions. Start with `event.acknowledge`; future dispatch/archive executors should reuse the same claim/complete/fail contract.
- `author_review_queue_items` is the durable operator queue for author intelligence. External ticketing or moderation systems can mirror this table while ThreadTrace keeps `status`, `lastSeenAt`, and source refs as the local source of truth.
