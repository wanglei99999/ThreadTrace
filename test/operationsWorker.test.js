...
test('operations worker can synthesize source type operations events before dispatch', async function () {
  const calls = [];
  const workerRuns = [];
  const worker = createOperationsWorker({
    logger: silentLogger(),
    ...
    runtime: {
      ...
      async synthesizeSourceTypeOperationsNotificationEvents(request) {
        calls.push(['source-type-operations-events', request.execute, request.sourceType, request.priorityScoreThreshold]);
        return {
          sourceTypeCount: 3,
          actionCount: 1,
          eventCount: 1,
  ...
  assert.deepEqual(calls, [
    ['sources'],
    ['source-type-operations-events', true, 'thread-url', 80],
    ['events'],
    ['overview']
  ]);
  assert.equal(result.sourceTypeOperationsEvents.eventCount, 1);
  assert.equal(workerRuns.at(-1).output.sourceTypeOperationsEvents.eventCount, 1);
  assert.equal(workerRuns.at(-1).output.sourceTypeOperationsEvents.priorityScoreThreshold, 80);
});
